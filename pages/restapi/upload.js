  ...
  const handleSubmit = async (e) => {
    ...
    if (file) {
      ...
      if (files && (files.type === 'image/png' || files.type === 'image/jpeg')) {
        ...
        googleUpload.createPicture(formData).then((res) => (res ? router.push('/restapi') : null))
        ...
        <div className='md:grid md:grid-cols-3 md:gap-6'>
          <div className='md:col-span-1'>
            <div className='px-4 sm:px-0'>
              <h3 className='text-lg font-medium leading-6 text-gray-900'>Photo Bucket</h3>
              <p className='mt-1 text-sm text-gray-600'>
                All the pictures uploaded here are stored in Cloud Storage Bucket.
              </p>
            </div>
          </div>
