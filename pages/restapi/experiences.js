    ...
    <div>
      ...
      <div className='mx-auto max-w-2xl py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8 '>
        <div className='flex justify-around'>
          <h2 className='text-2xl font-bold tracking-tight text-gray-900'>My Experiences</h2>
      ...
      >
        {experience ? (
          experience.map((exp, i) => (
            <div key={i} className='group relative border w-[500px] p-4 lg:h-[400px]'>
              <h2 className='font-bold text-[30px]'>{exp.company}</h2>
              <p className='text-md text-gray-500 mt-0'>{exp.position}</p>
              <p className='mt-4'>{exp.description}</p>
              <div className='flex justify-between items-end'>
                <p className='mt-4 text-gray-500'>
                  {new Date(exp.startDate).getFullYear()}/{new Date(exp.startDate).getMonth() + 1} -{' '}
                  {new Date(exp.endDate).getFullYear()}/{new Date(exp.endDate).getMonth() + 1}
                </p>
